...
/-! ### Signs of `a`, `b`, `c` and positivity of the norm form

In coordinates `x = (p, q)` the norm form is
`(p₀² - a p₁² - b p₂² + ab p₃²) + c (q₀² - a q₁² - b q₂² + ab q₃²)`
(`QuaternionAlgebra.re_star_mul_self_eq` for the quaternion factor, `re_mul_star_self` for the
doubling). So over a star-ordered `R` with trivial star (whose nonnegative cone is generated by the
squares), `a ≤ 0`, `b ≤ 0`, `0 ≤ c` make it a nonnegative combination of squares. -/

section Signs

variable {R : Type*} [CommRing R] {a b c : R}

/-- The norm of `p ∈ ℍ[R, a, 0, b]` in coordinates: `(star p * p).re = p₀² - a p₁² - b p₂² + ab p₃²`.
-/
theorem _root_.QuaternionAlgebra.re_star_mul_self_eq (p : ℍ[R, a, 0, b]) :
    (star p * p).re = p.re * p.re - a * (p.imI * p.imI) - b * (p.imJ * p.imJ) +
      a * b * (p.imK * p.imK) := by
  obtain ⟨p₀, p₁, p₂, p₃⟩ := p
  simp only [QuaternionAlgebra.star_mk, QuaternionAlgebra.mk_mul_mk]
  ring

variable [StarRing R] [TrivialStar R] [PartialOrder R] [StarOrderedRing R]

/-- With `a ≤ 0` and `b ≤ 0`, the quaternion norm `(star p * p).re` is nonnegative. -/
theorem _root_.QuaternionAlgebra.re_star_mul_self_nonneg (ha : a ≤ 0) (hb : b ≤ 0)
    (p : ℍ[R, a, 0, b]) : 0 ≤ (star p * p).re := by
  have sq : ∀ r : R, 0 ≤ r * r := fun r => by simpa [star_trivial] using star_mul_self_nonneg r
  rw [QuaternionAlgebra.re_star_mul_self_eq, sub_eq_add_neg, sub_eq_add_neg, ← neg_mul, ← neg_mul]
  exact add_nonneg (add_nonneg (add_nonneg (sq _) (mul_nonneg (neg_nonneg.mpr ha) (sq _)))
    (mul_nonneg (neg_nonneg.mpr hb) (sq _)))
    (mul_nonneg (mul_nonneg_of_nonpos_of_nonpos ha hb) (sq _))

/-- With `a ≤ 0`, `b ≤ 0` and `0 ≤ c`, the octonion norm form `(x * star x).1.re` is
nonnegative. -/
theorem norm_nonneg_of_signs (ha : a ≤ 0) (hb : b ≤ 0) (hc : 0 ≤ c) (x : Octonion R a b c) :
    0 ≤ (x * star x).1.re := by
  rw [re_mul_star_self]
  exact add_nonneg (QuaternionAlgebra.re_star_mul_self_nonneg ha hb _)
    (mul_nonneg hc (QuaternionAlgebra.re_star_mul_self_nonneg ha hb _))
