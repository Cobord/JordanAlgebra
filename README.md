...
| [`Jordan/Octonion.lean`](Jordan/Octonion.lean) | Generalized octonion algebras `Octonion R a b c` via Cayley-Dickson doubling, alternativity, and the self-adjoint (`1 x 1`) case. Also the `2 x 2` Hermitian octonionic case (the spin-factor identification via a trace/trace-free split, `ofSymmetricMatricesTwo`, complete), and the `3 x 3` Hermitian octonionic case (the exceptional Albert-algebra construction: `AlbertAlgebra`/`ofAlbert` stubbed, formal reality and its determinant currently `sorry`). |
