...
| [`Jordan/RealQM.lean`](Jordan/RealQM.lean) | Symmetric matrices over a base ring `R`, as a Jordan algebra; formal reality; the `n = 1` case. |
| [`Jordan/ComplexQM.lean`](Jordan/ComplexQM.lean) | "Complex" Hermitian matrices over `R` (via Mathlib's `QuadraticAlgebra`), generalizing the classical complex Hermitian case. |
| [`Jordan/QuaternionicQM.lean`](Jordan/QuaternionicQM.lean) | Quaternionic Hermitian matrices over `R` (via Mathlib's `QuaternionAlgebra`), plus the automorphism action of unit quaternions by conjugation. |
| [`Jordan/Octonion.lean`](Jordan/Octonion.lean) | Generalized octonion algebras `Octonion R a b c` via Cayley-Dickson doubling, alternativity, and the self-adjoint (`1 x 1`) case. Also the `2 x 2` Hermitian octonionic case (the spin-factor identification via a trace/trace-free split, `ofSymmetricMatricesTwo`, currently `sorry`), and the `3 x 3` Hermitian octonionic case (the exceptional Albert-algebra construction; not yet developed). |
| [`Jordan/SpinFactor.lean`](Jordan/SpinFactor.lean) | The spin factor Jordan algebra `V × R` from a symmetric bilinear form `B` on `V`, its determinant, and formal reality under positive definiteness. |
