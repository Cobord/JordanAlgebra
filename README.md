...
formal reality, Jordan triple systems, and the structure algebra of derivations. Alongside these,
[`Jordan/Triality/`](Jordan/Triality/) develops trialities (trilinear forms whose slices are
perfect pairings), the non-associative division algebras they cut out, and the octonions and
finite fields as examples.
...
| [`Jordan/Octonion.lean`](Jordan/Octonion.lean) | Generalized octonion algebras `Octonion R a b c` via Cayley-Dickson doubling, `IsAlternative`, the octonion norm (`mul_star_self_eq_scalarEmbed`) and its positive-definiteness, the self-adjoint (`1 x 1`) case, the `innerProduct` bilinear form with its multiplicative "adjoint" identities (`innerProduct_mul_left/right` and friends), and `nuclearInvolution`: for `a`, `b`, `c` non-zero-divisors, `Octonion R a b c` is a nuclear involution (`nuclear_rpart` proves Nuc ⊆ Center via coordinate tests against the Cayley-Dickson basis), taking that regularity as an explicit hypothesis rather than as a global instance. Finally the norm form in coordinates (`QuaternionAlgebra.re_star_mul_self_eq`) and its nonnegativity from the signs `a ≤ 0`, `b ≤ 0`, `0 ≤ c` over a star-ordered ring (`norm_nonneg_of_signs`). |
...
| [`Jordan/DivisionAlgebra.lean`](Jordan/DivisionAlgebra.lean) | `DivisionAlgebra R K`: unital, not necessarily associative `R`-algebras in which multiplication by any nonzero element on either side is bijective (Mathlib's `DivisionRing` is the associative case). Bundles `NonAssocRing`, `Module R`, `IsScalarTower`/`SMulCommClass`, `NoZeroDivisors`, `Nontrivial`; `ofBijective` derives `NoZeroDivisors` from the bijectivity axioms; `ofDivisionRing`/`ofField` for the associative case. |
| [`Jordan/Triality/`](Jordan/Triality/) | Trialities `t : M1 × M2 × M3 → R` (antilinear in `M2`) whose slices at nonzero elements are perfect pairings; the reduction along idempotents to a corner ring; the division algebras cut out by choosing two distinguished elements; gauged refinements and Baez's normed trialities (inner products, `|t x y z|² ≤ ⟪x,x⟫ ⟪y,y⟫ ⟪z,z⟫` with the bound attained in each slot); and the octonion and finite-field examples. See the [subfolder README](Jordan/Triality/README.md). |
